...
namespace App\Http\Controllers\Admin\ApprovalController\SeniorMdDoctor;
...
class ApprovalSeniorMdController extends Controller
{
    public function show()
    {
        $disbar = 'approval.seniormddoctor';
        return view('admin.approval.seniormddoctor.seniormdlist', compact('disbar'));
    }
    public function ajaxlist(Request $request)
    {
        ...
        foreach ($bookings as $books) {
            $userprofile = '<a href="' . url('admin/approval/seniormdprofile/' . base64_encode($books->id)) . '" target="_blank" class="btn btn-info  badge-info "><i class="fa fa-user-circle-o"></i></a>';

            $status = '<button class="status_item btn-sm btn ' . ($books->approval_by_admin == 1 ? "btn-success" : "btn-danger") . '  " data-status="' . ($books->approval_by_admin == 1  ? '0' : '1') . '" data-id="' . $books->id . '">' . ($books->approval_by_admin == 1 ?  "Approved" : "Not-Approved") . '</button>';
            $data[] = array(
                $i++,
                $books->name,
    ...
    public function status(Request $request)
    {
        ...
        if ($updatedata) {
            Helper::addToActivities('CHANGE OF APPROVAL STATUS SENIORMD SUCCESS-' . Auth::guard('admin')->user()->id, Auth::guard('admin')->user()->id, 'ADMIN', $request->fullUrl(), $request->ip(), 'POST');

            return response()->json(['status' => true, 'msg' => " Approval Staus Change Successfully"]);
            exit;
        } else {
            Helper::addToActivities('CHANGE OF APPROVAL STATUS SENIORMD FAILE  -' . Auth::guard('admin')->user()->id, Auth::guard('admin')->user()->id, 'ADMIN', $request->fullUrl(), $request->ip(), 'POST');
    ...
    public function profile(Request $request)
    {
        ...
        return view('admin.approval.seniormddoctor.profile', compact('disbar', 'details', 'educatioDetails', 'specilisation'));
    }
    ...
    public function educationAjax(Request $request)
    {
        ...
        })->where('doctor_id', $request->id)->where('type', 'seniormd')
            ->get();
